...
atoms = {
	...
	'Ṙ': attrdict(
		arity = 1,
		call = lambda z: output(z, transform = confiture_uneval)
	),
	'r': attrdict(
		arity = 2,
		ldepth = 0,
	...
	'V': attrdict(
		arity = 1,
		ldepth = 1,
		call = lambda z: confiture_eval(''.join(map(str, z)), [])
	),
	'Ṿ': attrdict(
		arity = 1,
		call = lambda z: listify(confiture_uneval(z))
	),
	'v': attrdict(
		arity = 2,
		ldepth = 1,
		call = lambda x, y: confiture_eval(''.join(map(str, x)), [y])
	),
	'W': attrdict(
		arity = 1,
		call = lambda z: [z]
